...
# '''
import unittest
from pages.update_person_page import UpdatePersonBusiness
import ddt
...
class TestDrug(unittest.TestCase):
    ...
    '''
     ...
     '''
    #从Excel表格中读取数据
    infos = ReadData.readExcel('mms_ok','修改经办人信息')
    #大列表分为小列表
    @ddt.data(*infos)
    #小列表的数据打散，传给每一个形参
    @ddt.unpack
    @screenshot.add_pic
    #测试修改经办人。注意：要传入列表的数据给形参，这里的函数必须要有参数
    def test_updateok_person(self,id,personid,name,sex,phone,tip,msg_text):
        # 获取pass的图片
        self.driver = DriverFactory.get_driver()
        self.imgs = []

        #先进行类的实例化
        d = UpdatePersonBusiness()
        #调用新增的方法，获取返回值
        r = d.UpdatePerson(id,personid,name,sex,phone,tip)
    ...
    '''
    #从Excel表格中读取数据
    infos = ReadData.readExcel('mms_eg','修改经办人信息')
    #大列表分为小列表
    @ddt.data(*infos)
    #小列表的数据打散，传给每一个形参
    @ddt.unpack
    @screenshot.add_pic
    #测试修改经办人。注意：要传入列表的数据给形参，这里的函数必须要有参数
    def test_updatefail_person(self,id,personid,name,sex,phone,tip,msg_text):
        # 获取pass的图片
        self.driver = DriverFactory.get_driver()
        self.imgs = []
        ...
        d = UpdatePersonBusiness()
        #调用新增的方法，获取返回值
        f = d.UpdatePerson(id,personid,name,sex,phone,tip)
        # 截图--htmltestrunner框架固定的写法
        self.imgs.append(self.driver.get_screenshot_as_base64())
        # 断言：如果预期结果在返回值里则成功
